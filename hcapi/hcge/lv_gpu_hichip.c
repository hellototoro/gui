...
static bool lv_draw_hichip_blend_fill(lv_color_t * dest_buf, lv_coord_t dest_stride, const lv_area_t * fill_area,
                                      lv_color_t color, const lv_color_t * mask_buf,const lv_area_t * mask_area, lv_coord_t mask_stride);
...
static bool lv_draw_hichip_blend_map(lv_color_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
                                     ...
                                     const lv_color_t * mask_buf,const lv_area_t * mask_area, lv_coord_t mask_stride,
                                     lv_opa_t opa);
                                      ...
                                      lv_color_t color, const lv_color_t * mask_buf,const lv_area_t * mask_area, lv_coord_t mask_stride)
{
	...
	state->dst.phys = fbdev_buffer_addr_virt_to_phy(dest_buf);
	...
	if(mask_buf && mask_stride && mask_area) {
		...
		state->src_mask.phys = fbdev_buffer_addr_virt_to_phy(mask_buf);
                                     ...
                                     lv_opa_t opa)
{
	...
	state->src.phys = fbdev_buffer_addr_virt_to_phy(src_buf);
