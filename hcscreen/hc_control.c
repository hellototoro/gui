...
static uint8_t detect_wifi_reset_press(uint32_t timeout)
{
    int time_cnt = 0;
    int gpio_val = -1;
    time_cnt = timeout/10;
...
static void key_wifi_reset_proc()
{

    uint8_t wifi_param_reset = 0;
    wifi_param_reset = detect_wifi_reset_press(1500);
    control_msg_t msg;
...
static void *key_monitor_task(void *arg)
{
    for(;;)
    {
        ...
        if (count ++ < 100)
            gpio_write(g_gpio_reset_hld, true);
        else if (count ++ < 200)
            gpio_write(g_gpio_reset_hld, false);
        else
            count = 0;
...
// check if need to connect wifi
static bool hc_control_connect_wifi()
{
